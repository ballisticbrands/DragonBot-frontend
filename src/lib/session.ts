// Client-side session helper. Stores the auth token in localStorage and
// exposes a React hook for components to consume the current user.
//
 ...
 * or `anonymous`. Components that need an authed user can call
 * `useRequireSession()` instead — it redirects to /sign-in when needed.
 */
