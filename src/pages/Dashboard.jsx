    ...
    <div className={`min-h-screen flex items-center justify-center px-4 ${dark ? 'bg-[#0f0f0f]' : 'bg-[#fafafa]'}`}>
      <div className={`w-full max-w-sm rounded-2xl border p-8 shadow-xl ${dark ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-gray-200'}`}>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <p className={`font-medium text-sm ${dark ? 'text-white' : 'text-[#1A1A1A]'}`}>
              {displayName}
            </p>
            {session?.email && (
              <p className={`text-xs ${dark ? 'text-white/40' : 'text-[#1A1A1A]/40'}`}>
                {session.email}
              </p>
            )}
          </div>
        ...
        {hasBot ? (
          <div>
            <div className={`rounded-xl p-4 mb-4 ${dark ? 'bg-[#2F7D4F]/10 border border-[#2F7D4F]/20' : 'bg-[#2F7D4F]/5 border border-[#2F7D4F]/10'}`}>
              <p className={`text-sm font-medium mb-1 ${dark ? 'text-white' : 'text-[#1A1A1A]'}`}>
                Your DragonBot is ready
              </p>
              <p className={`text-xs ${dark ? 'text-white/50' : 'text-[#1A1A1A]/50'}`}>
                Start chatting with your AI assistant.
              </p>
            </div>
            <a
              href="#/chat"
              className="block w-full py-2.5 rounded-xl bg-[#2F7D4F] hover:bg-[#256B42] text-white text-sm font-medium transition-colors shadow-lg shadow-[#2F7D4F]/20 text-center"
            >
              Open Chat
            </a>
          </div>
        ...
        <button
          ...
          className={`mt-6 w-full py-2 rounded-xl text-sm font-medium transition-colors border ${
            dark
